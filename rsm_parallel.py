...
def rsm(path: str, fnamepng: str, cmap='jet', opacity='linear', md: int = 0, nd: int = 0, el: int = 0, az: int = 0):
    ...
    # Load TIFF files concurrently.
    print("Reading TIFF files concurrently...")
    def load_file(fname):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        images_list = list(executor.map(load_file, tifFiles))
    print(f"Completed reading {N} files.")
    
    # Stack images into a 3D volume array, shape: (Ny, Nx, N)
    A = np.stack(images_list, axis=2)
    
    # Compute the per-slice angles in a vectorized way.
    th = th0 + np.arange(N) * dth  # shape: (N,)
    # Vectorized computation for dQ arrays:
    # Expand Qnm_x and Qnm_z to 3D and apply broadcasting using np.newaxis.
    dQx = Qnm_x[..., np.newaxis] * np.cos(th)[np.newaxis, np.newaxis, :] + \
          Qnm_z[..., np.newaxis] * np.sin(th)[np.newaxis, np.newaxis, :]
    dQz = - Qnm_x[..., np.newaxis] * np.sin(th)[np.newaxis, np.newaxis, :] + \
           Qnm_z[..., np.newaxis] * np.cos(th)[np.newaxis, np.newaxis, :]
    # Qnm_y remains constant; repeat it along the third axis.
    dQy = np.repeat(Qnm_y[..., np.newaxis], N, axis=2)
    ...
    # Create a structured grid using the dQ arrays.
    mesh = pv.StructuredGrid(dQx, dQy, dQz)
    # Ensure data order (Fortran) for VTK.
    mesh.point_data['values'] = Z.ravel(order='F')
    # Compute isosurfaces based on scale factors.
    ...
    plotter.show_grid(xlabel="Qx", ylabel="Qy", zlabel="Qz", color="green", font_family="arial")
...
if __name__ == "__main__":
    # Replace this with your TIFF folder path.
